/**
 * Logi Native Storage Adapter (Bridge Master Healer v162)
...
(function() {
  ...
  function __debug(msg) {
    if (window.__logiDebug) window.__logiDebug(`[Bridge] ${msg}`);
  ...
  async function healStorageSplit() {
    ...
    __debug("Bridge Master Scavenger Start...");
    ...
    try {
      ...
      _migrationDone = true;
      __debug(`Bridge Master DONE. Assets moved: ${totalMovedCount}`);
    } catch (e) { 
       ...
       for(const t of paths){
         try {
           ...
           if (response.ok) {
              ...
              __debug(`SUCCESS: Report blob ${t.p} loaded (${b.size} bytes)`);
              return b;
           }
         }catch(e){}
       }
       __debug(`FAILURE: Report NOT found for ID: ${searchId}`);
       return null;
    },
