...
async function shareBlobAsFile(blob, filename, mime, title, opts) {
  const options = opts || {};
  const fallbackDownload = options.fallbackDownload === true;

  const safeName = filename || "archivo";
  const rawMime = (mime || blob?.type || "").trim();
  const lower = String(safeName || "").toLowerCase();
  const isDocx = lower.endsWith(".docx") || /wordprocessingml/i.test(rawMime);
  const isPdf = lower.endsWith(".pdf") || /pdf/i.test(rawMime);
  const isZip = lower.endsWith(".zip") || /zip/i.test(rawMime);
...
function setTab(mode){
  ...
  if (mode === "informes"){
    ...
    loadReportsForProject(pid).then(()=>{ if (viewMode === "informes") renderInformes(); });
  }
...
function renderInformes(){
  ...
  wrap.innerHTML = list.map((r, idx)=>{
    const rangeLabel = fmtRangeLabelES(r.start, r.end);
    return `
...
function openReportViewer(r){
  ...
  meta.textContent = fmtRangeLabelES(r.start, r.end);
