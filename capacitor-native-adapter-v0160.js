/**
 * Logi Native Storage Adapter (Deep Fisher Healer v160)
 * - Deep Scavenger: Now also recovers .pdf and .docx reports.
 * - Auto-Index: Reconstructs report metadata if the Web IndexedDB is empty.
 * - Scoped Storage: Uses block-by-block read/write for all asset types.
 */
(function() {
  const VERSION = "v0160";
  ...
  function __debug(msg) {
    if (window.__logiDebug) window.__logiDebug(`[Fisher] ${msg}`);
  ...
  async function healStorageSplit() {
    ...
    try {
        ...
        try {
          ...
          if (files.length > 0) {
            ...
            for (const name of files) {
               ...
               try {
                  // Scoped storage friendly move: read+write
      ...
      _migrationDone = true;
      __debug(`Deep Scavenger DONE. Assets moved: ${totalMovedCount}`);
    } catch (e) { 
    ...
    try {
      ...
      for (const name of files) {
         if (name.endsWith('.pdf') || name.endsWith('.docx')) {
            ...
            // Attempt to find metadata JSON
            let meta = { id, name, createdAt: Date.now(), projectId: filterPid || 'unknown' };
            try {
  ...
  window.LogiNative = {
    ...
    dbGet: async (s, id) => {
       ...
       try {
         const r = await Filesystem.readFile({ path: DATA_DIR + '/' + s + '/' + id + ext, directory: PRIMARY_DIR, encoding: 'utf8' });
         return JSON.parse(r.data);
       } catch(e){
         try {
           const r2 = await Filesystem.readFile({ path: DATA_DIR + '/' + s + '/' + id + ext, directory: FALLBACK_DIR, encoding: 'utf8' });
    ...
    getBlob: async (id) => {
      try {
        const r = await Filesystem.readFile({ path: DATA_DIR + '/blobs/' + id + '.jpg', directory: PRIMARY_DIR });
        return new Blob([Uint8Array.from(atob(r.data), c => c.charCodeAt(0))], { type: 'image/jpeg' });
      } catch (e) {
        try {
          const r2 = await Filesystem.readFile({ path: DATA_DIR + '/blobs/' + id + '.jpg', directory: FALLBACK_DIR });
