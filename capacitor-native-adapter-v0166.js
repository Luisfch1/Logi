/**
 * Logi Native Storage Adapter (Titan Guardian v166)
 * - Titan Guardian: Uses Filesystem.copy for memory-safe migration (fixes OOM crashes).
 * - Name Guardian: Protects file extensions and original names.
 * - Master Healer Plus: Improved MIME detection.
 */
(function() {
  const VERSION = "v0166";
        ...
        try {
          ...
          if (files.length > 0) {
            ...
            for (const name of files) {
               ...
               try {
                  // v166: Using Filesystem.copy is 100% memory safe for large files
                  ...
                  // Fallback to manual if copy fails (though copy is preferred)
                  try {
      ...
      for (const name of files) {
         if (name.endsWith('.pdf') || name.endsWith('.docx')) {
            ...
            try {
               ...
               // Preserve pretty name but ensure extension exists
               if (jsonMeta.name && jsonMeta.name !== id) {
                  meta.name = jsonMeta.name.includes('.') ? jsonMeta.name : jsonMeta.name + ext;
               }
               if (jsonMeta.projectId) meta.projectId = jsonMeta.projectId;
               if (jsonMeta.createdAt) meta.createdAt = jsonMeta.createdAt;
            } catch(e){}
  ...
  window.LogiNative = {
    ...
    dbGet: async (s, id) => {
       ...
       try {
         ...
         if(s==='reports') { 
            obj.scavenged = true; 
            if (!obj.mime) obj.mime = isWord ? 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' : 'application/pdf'; 
         ...
         try {
           ...
           return obj;
            ...
            if (s === 'reports') return { id: searchId, name: searchId + fallbackExt, scavenged: true, projectId: 'unknown', mime: isWord ? 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' : 'application/pdf' };
            return null; 
         }
       }
    },
    ...
    getReport: async (id) => {
       ...
       __debug(`getReport: Request ID<sup>+</sup> ${searchId}`);
       for(const t of paths){
         try {
           const res = await Filesystem.getUri({ path: t.p, directory: t.d });
           const url = Capacitor.convertFileSrc(res.uri);
           __debug(`  Trying<sup>+</sup> ${t.d}: ${t.p}`);
           const response = await fetch(url);
           if (response.ok) {
              const b = await response.blob();
              __debug(`  -> SUCCESS<sup>+</sup>: ${b.size} bytes`);
              return b;
           }
         }catch(e){}
       }
       __debug(`  -> FAILURE<sup>+</sup>: Report not found.`);
       return null;
    },
