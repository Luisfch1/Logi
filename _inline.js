...
function syncSkinUI(){
  const key = (localStorage.getItem(SKIN_KEY) || "glass");
...
function renderTemplateCards(list){
  ...
  const all = [
    ...list,
    { id:"soon", name:"Plantilla 3", subtitle:"Próximamente", preview:"soon", disabled:true }
  ];
...
async function openTplModal(id){
  ...
  try{
    const t = (_tplCatalog || []).find(x => x.id === id);
  ...
  body.innerHTML = '<div class="muted">Generando previsualización…</div>';
  ...
  try{
    const blob = await buildTemplatePreviewPdfBlob(id);
    if (!blob){ body.innerHTML = '<div class="muted">No pude generar el preview.</div>'; return; }
    const url = URL.createObjectURL(blob);
    _tplModalBlobUrl = url;
    body.innerHTML = `
      <div class="tplPdfWrap"><iframe class="tplPdfFrame" src="${url}#zoom=page-width" title="Preview PDF"></iframe></div>
      <div class="tplPdfHint">Preview real (PDF). Puede tardar un poco según el dispositivo.</div>
    `;
  }catch(e){
    body.innerHTML = '<div class="muted">Error generando preview: ' + tplEscapeHtml(e?.message||String(e)) + '</div>';
  }
}
  ...
  if (onlyDone) selected = selected.filter(x => !!x.done);
    ...
    if (fmt === "docx") {
      if (reportTpl === "tags") {
        ...
        if (zi){ zi.textContent = `Generando Word (Plantilla 2)... 0/${total}`; await nextPaint(); }

        const onProgress = async (done, tot) => {
          if (!zi) return;
          zi.textContent = `Generando Word (Plantilla 2)... ${done}/${tot}`;
          await nextPaint();
        };
