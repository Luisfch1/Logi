/**
 * Logi Native Storage Adapter (Star Shield v169)
 * - Star Shield: Throttled scavenger migration (prevents UI freezing/ANRs).
 * - Omni Shield+: Absolute filename integrity for sharing.
 * - Project Map+: Robust numeric ID handling.
 */
(function() {
  const VERSION = "v0169";
  if (window.__logiDebug) window.__logiDebug(`!!! ADAPTER ${VERSION} LOADED (Star Shield) !!!`);
  ...
  async function healStorageSplit() {
    ...
    try {
          ...
          if (files.length > 0) {
            ...
            for (const name of files) {
               ...
               try {
                  ...
                  // Throttling: batched by 100 to prevent ANR/Bridge Overload
                  if (count % 100 === 0) {
                     __debug(`${store}: ${count}/${files.length} ready...`);
      ...
      _migrationDone = true;
      __debug(`Star Migration DONE. Moved: ${totalMovedCount}`);
    } catch (e) { 
