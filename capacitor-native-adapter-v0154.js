/**
 * Logi Native Storage Adapter (The Resurrection Edition)
 * Version: 2026.03.20.0154 (BROAD SEARCH & RECOVERY)
 * This version performs an exhaustive search in root and subdirectories to find all data.
 */
(function() {
  if (window.__logiDebug) window.__logiDebug("!!! ADAPTER v0154 LOADED !!!");
  console.log("DEBUG: capacitor-native-adapter (v0154) resurrection start");
  ...
  const PROJECT_FOLDERS = ['Logi', '']; // Search in "Logi/" AND the root folder
  ...
  async function resurrectionReadFile(storePath) {
    // Try all folder/dir combinations
    ...
    throw new Error("File not found anywhere: " + storePath);
  }

  async function resurrectionGetUri(storePath) {
    for (const folder of PROJECT_FOLDERS) {
       for (const dir of DIRS) {
          try {
             const path = (folder ? folder + '/' : '') + storePath;
    ...
    throw new Error("Uri not found anywhere: " + storePath);
  }

  async function resurrectionReaddir(storeSubPath) {
    let allFiles = [];
  ...
  async function loadMetaDb() {
    ...
    if (!_cachedBlobBaseUrl) {
       try {
         const result = await resurrectionGetUri('blobs');
         let base = result.uri;
    ...
    if (foundAnyMaster && merged.length > 0) {
       _nativeMetaCache = merged;
       return _nativeMetaCache;
    }

    // BROAD REPAIR: Scan all stores in all locations
    const items1 = await scanResurrectionFiles('meta');
    const items2 = await scanResurrectionFiles('items_meta');
    _nativeMetaCache = [...items1, ...items2];
    if (_nativeMetaCache.length > 0) await saveMetaDb();
    return _nativeMetaCache;
  }

  async function scanResurrectionFiles(storeName) {
    if (!Filesystem) return [];
    try {
      const res = await resurrectionReaddir(storeName);
      const files = res.files || [];
      const items = [];
      const queue = files.map(f => (typeof f === 'string') ? f : f.name).filter(n => n.endsWith('.json'));
      const concurrency = 30; // High speed scan
      const workers = [];
  ...
  async function nativeDbGet(storeName, id) {
    ...
    try {
      const result = await resurrectionReadFile(storeName + '/' + id + '.json');
  ...
  async function nativeDbGetAll(storeName) {
    ...
    return await scanResurrectionFiles(storeName);
  }
  ...
  async function nativeGetBlob(id) {
    if (!Filesystem) return null;
    try {
      const result = await resurrectionReadFile('blobs/' + id + '.jpg');
  ...
  async function nativeShare(blob, filename, mime, title, id) {
    ...
    try {
      ...
      if (id) {
          ...
          try {
             const res = await resurrectionGetUri('reports/' + safeId + ext);
             finalUri = res.uri;
          } catch(e){}
      }
      if (!finalUri) {
  ...
  async function nativeGetBlobUrl(id) {
    if (!Filesystem) return null;
    try {
      const result = await resurrectionGetUri('blobs/' + id + '.jpg');
  ...
  async function nativeGetReport(id) {
    ...
    try {
      const safeId = String(id).replace(/[^a-z0-9_-]/gi, '_');
      const types = [
          {p: 'reports/' + safeId + '.pdf', t: "application/pdf"},
          {p: 'reports/' + safeId + '.docx', t: "application/vnd.openxmlformats-officedocument.wordprocessingml.document"}
      ];
      for (const ty of types) {
         try {
            const res = await resurrectionReadFile(ty.p);
            return base64ToBlob(res.data, ty.t);
         } catch(e){}
      }
      return null;
    } catch (e) { return null; }
