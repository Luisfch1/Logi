/**
 * Logi Native Storage Adapter (Name Guardian v165)
 * - Name Guardian: Protects file extensions and original names during recovery.
    ...
    try {
      ...
      for (const name of files) {
         if (name.endsWith('.pdf') || name.endsWith('.docx')) {
            ...
            let meta = { id, name, createdAt: Date.now(), projectId: filterPid || 'unknown', scavenged: true, mime: mime };
            
            try {
               const r = await Filesystem.readFile({ path: path + '/' + id + '.json', directory: dirType, encoding: 'utf8' });
               const jsonMeta = JSON.parse(r.data);
               // If JSON found, preserve pretty name but ensure extension exists
