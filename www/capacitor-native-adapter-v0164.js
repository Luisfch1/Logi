/**
 * Logi Native Storage Adapter (Master Healer Plus v164)
 * - Master Healer Plus: Improved MIME detection for recovered reports.
  ...
  window.LogiNative = {
    ...
    dbGet: async (s, id) => {
       ...
       try {
         ...
         const obj = JSON.parse(r.data); if(s==='reports') { obj.scavenged = true; if (!obj.mime) obj.mime = searchId.includes('docx') ? 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' : 'application/pdf'; }
         return obj;
       } catch(e){
         try {
           const r2 = await Filesystem.readFile({ path: DATA_DIR + '/' + s + '/' + searchId + ext, directory: FALLBACK_DIR, encoding: 'utf8' });
           const obj = JSON.parse(r2.data); if(s==='reports') { obj.scavenged = true; if (!obj.mime) obj.mime = searchId.includes('docx') ? 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' : 'application/pdf'; }
           return obj;
         } catch(e2){ 
            if (s === 'reports') return { id: searchId, name: searchId, scavenged: true, projectId: 'unknown', mime: searchId.includes('docx') ? 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' : 'application/pdf' };
            return null; 
         }
       }
    },
