/**
 * Logi Native Storage Adapter (Supernova Shield v171)
 * - Supernova Shield: Legacy compatibility bridge (getBlobBaseUrlSync) + Nova Scavenger.
 * - Performance: Throttled batches of 50 with 100ms yields + Persistent Lock.
 * - Stability: Fixes TypeError crash for legacy UI calls in mixed-cache environments.
 */
(function() {
  const VERSION = "v0171";
  if (window.__logiDebug) window.__logiDebug(`!!! ADAPTER ${VERSION} LOADED (Supernova Shield) !!!`);
  ...
  const MIGRATION_LOCK = DATA_DIR + '/migration_v170.done'; // Reuse 170 lock

  // Discovery: Dynamic Base URI for thumbnails
  ...
  async function discoverBlobsUri() {
    ...
    try {
      ...
      _dynamicBlobsUri = Capacitor.convertFileSrc(res.uri) + '/';
      __debug(`DISCOVERED BLOBS URI: ${_dynamicBlobsUri}`);
    } catch(e) { __debug(`DISCOVERY FAILED: ${e.message}`); }
  }
  discoverBlobsUri();
  ...
  // --- MIGRATION ENGINE (v171 - Inherited Throttling & Persistence) ---
  async function healStorageSplit() {
    ...
    try {
       ...
       __debug("Supernova: Persistent lock found. Skipping scavenger.");
       _migrationDone = true;
       return;
    } catch(e){}

    _migrationActive = true;
    __debug("Supernova Scavenger Start (Throttled 50ms/100ms)...");
  ...
  window.LogiNative = {
    isNative: () => true,
    getBlobBaseUrlSync: () => _dynamicBlobsUri || SYNC_BLOBS_URI, 
    dbPut: async (s, it) => {
