/**
 * Logi Native Storage Adapter (Blob Master Healer v161)
...
(function() {
  ...
  // --- BLOB MASTER MIGRATION ---
  async function healStorageSplit() {
    if (_migrationActive || _migrationDone || !Filesystem) return;
    _migrationActive = true;
    __debug("Blob Master Scavenger Start...");
    
    try {
      // PRIORITIZED ORDER: reports and meta first
      ...
      _migrationDone = true;
      __debug(`Blob Master DONE. Assets moved: ${totalMovedCount}`);
    } catch (e) { 
  ...
  window.LogiNative = {
    ...
    getReport: async (id) => {
       const paths = [
         {p:DATA_DIR+'/reports/'+id+'.pdf', d:PRIMARY_DIR}, {p:DATA_DIR+'/reports/'+id+'.docx', d:PRIMARY_DIR},
         {p:DATA_DIR+'/reports/'+id+'.pdf', d:FALLBACK_DIR}, {p:DATA_DIR+'/reports/'+id+'.docx', d:FALLBACK_DIR}
       ];
       for(const t of paths){
         try {
           ...
           if (response.ok) {
              ...
              __debug(`Found report blob: ${t.p} (${b.size} bytes)`);
              return b;
           }
         }catch(e){}
       }
       __debug(`Report NOT found in any loc for ID: ${id}`);
       return null;
    },
