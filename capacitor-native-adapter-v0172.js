/**
 * Logi Native Storage Adapter (Pulsar Shield v172)
 * - Pulsar Shield: Dynamic URI discovery for internal thumbnails.
 * - Compatibility: Re-adds getBlobBaseUrlSync with runtime path detection.
 * - Performance: Inherits Nova Throttling (50/100ms) and Persistent Lock.
 */
(function() {
  const VERSION = "v0172";
  if (window.__logiDebug) window.__logiDebug(`!!! ADAPTER ${VERSION} LOADED (Pulsar Shield: Active) !!!`);
  ...
  // Dynamic URI Discovery for thumbnails
  ...
  async function discoverBlobsUri() {
    ...
    try {
      ...
      __debug(`Pulsar: Discovered internal blobs URI: ${_dynamicBlobsUri}`);
    } catch(e) { __debug(`Pulsar: Discovery failed, using legacy fallback.`); }
  }
  discoverBlobsUri();
  ...
  async function healStorageSplit() {
    ...
    try {
       ...
       __debug("Pulsar: Persistent lock found. Migration skipped.");
       _migrationDone = true;
       return;
    } catch(e){}

    _migrationActive = true;
    __debug("Pulsar Scavenger Start...");
    try {
          ...
          if (files.length > 0) {
            ...
            for (const name of files) {
               ...
               try {
                  ...
                  if (count % 50 === 0) {
                     __debug(`Pulsar ${store}: ${count}/${files.length} processed...`);
