...
/* ===========================
   Recordatorio diario de Backup TOTAL
=========================== */
function maybeShowBackupReminder(){
  try{
    ...
    if (lastBackupAll === today) return; // ya hizo backup total hoy
    if (lastShown === today) return;     // ya se mostró hoy
    ...
    // Un mini delay para que se sienta menos invasivo
    setTimeout(show, 650);
  }catch(e){}
}
...
async function shareBlobAsFile(blob, filename, mime, title, opts) {
  ...
  const safeMime = mime || blob?.type || "application/octet-stream";
  const file = new File([blob], safeName, { type: safeMime });
  ...
  if (navigator.share) {
    try {
      ...
      // Si no queremos descargar automáticamente, salimos.
      if (!fallbackDownload) {
        console.warn("share failed (no fallback download):", e);
        return false;
      }
      // Si hay fallback, seguimos a descarga.
    }
  }
  ...
  if (onlyDone) selected = selected.filter(x => !!x.done);
    ...
    if (fmt === "docx") {
      if (reportTpl === "tags") {
        ...
        const defBaseW = `Logi__${sanitizeName(tituloProyecto || "Proyecto")}__${start}_a_${end}${itemTag}__WORD_PLANTILLA_2`;
      ...
      if (tplRec && tplRec.ab){
        ...
        const activeIdW = getActiveProjectId() || ensureProjects().activeId;
        const defBaseW = `Logi__${sanitizeName(tituloProyecto || "Proyecto")}__${start}_a_${end}${itemTag}__WORD_USUARIO`;
        const reportNameW = await askReportName(defBaseW, ".docx");
        if (!reportNameW){ exportStatusSet(""); workModalHide(); return; }
      ...
      const activeIdW = getActiveProjectId() || ensureProjects().activeId;
      const defBaseW = `Logi__${sanitizeName(tituloProyecto || "Proyecto")}__${start}_a_${end}${itemTag}__WORD`;
      const reportNameW = await askReportName(defBaseW, ".docx");
      if (!reportNameW){ exportStatusSet(""); workModalHide(); return; }
      try{
        await repPut({
    ...
    const defBaseP = `Logi__${sanitizeName(tituloProyecto || "Proyecto")}__${start}_a_${end}${itemTag}__${sanitizeName(tplLabel)}`;
...
async function loadReportsForProject(projectId){
  try{
    const list = await repGetAllByProject(projectId);
    // sort desc
    list.sort((a,b)=> (b.createdAt||0)-(a.createdAt||0));
    _reportsCache.set(projectId, list);
    return list;
  }catch(e){
    console.warn("loadReportsForProject failed", e);
    _reportsCache.set(projectId, []);
    return [];
  }
}
...
function renderInformes(){
  ...
  const cached = _reportsCache.get(activeId);
  if (!cached && !_reportsLoading){
  ...
  for (const r of list){
    ...
    const when = dt ? dt.toLocaleString("es-CO") : "—";
    const size = fmtBytes(r.sizeBytes || (r.blob ? r.blob.size : 0));
    const range = (r.start && r.end) ? `${r.start}${(r.end && r.end!==r.start)?(" a "+r.end):""}` : "";
    const tpl = r.templateLabel ? ` · ${r.templateLabel}` : "";
    html += `
      <div class="card" style="margin-top:10px">
        <div class="row" style="justify-content:space-between;align-items:flex-start;gap:10px">
          <div style="min-width:0">
            <div style="font-weight:900;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${escapeHtml(r.name||"Informe")}</div>
            <div class="muted" style="margin-top:2px">${escapeHtml(when)}${range?(" · "+escapeHtml(range)):""}${tpl}</div>
            <div class="muted" style="margin-top:2px">${escapeHtml(r.mime||"")} · ${escapeHtml(size)}</div>
          </div>
          <div style="display:flex;gap:8px;flex-wrap:wrap;justify-content:flex-end">
            <button class="btn btn-secondary smallBtn repView" data-id="${escAttr(r.id)}" type="button">👁️</button>
            <button class="btn btn-secondary smallBtn repShare" data-id="${escAttr(r.id)}" type="button">📤</button>
            <button class="btn btn-secondary smallBtn repRename" data-id="${escAttr(r.id)}" type="button">✏️</button>
            <button class="btn btn-danger smallBtn repDelete" data-id="${escAttr(r.id)}" type="button">🗑️</button>
          </div>
        </div>
      </div>
    `;
  ...
  wrap.querySelectorAll(".repView").forEach(b=>{
    b.onclick = ()=>{
      const id = String(b.dataset.id || "");
      const r = byId.get(id);
      if (r) openReportViewer(r);
      else openReportViewerById(id);
    };
  });

  wrap.querySelectorAll(".repShare").forEach(b=>{
    b.onclick = async ()=>{
      const id = String(b.dataset.id || "");
      const r = byId.get(id) || await repGet(id);
      if (!r || !(r.blob instanceof Blob)){ alert("No encontré ese informe."); return; }
      const ok = await shareBlobAsFile(
        r.blob,
        r.name || "Informe",
        r.mime || r.blob.type || "application/octet-stream",
        "Logi - Informe",
        { fallbackDownload:false }
      );
      if (!ok) alert("No pude abrir el panel de compartir en este dispositivo. Usa Descargar.");
    };
  });

  wrap.querySelectorAll(".repRename").forEach(b=>{
    b.onclick = async ()=>{
      const id = b.dataset.id;
      const r = byId.get(String(id)) || await repGet(id);
      if (!r) return;
      const ext = (r.name && r.name.toLowerCase().endsWith(".docx")) ? ".docx" : (r.name && r.name.toLowerCase().endsWith(".pdf")) ? ".pdf" : "";
      const newName = await askReportName((r.name||"Informe").replace(/\.(pdf|docx)$/i,""), ext);
      if (!newName) return;
      r.name = newName;
      await repPut(r);
      _reportsCache.delete(r.projectId);
      if (viewMode === "informes") renderInformes();
    };
  });

  wrap.querySelectorAll(".repDelete").forEach(b=>{
    b.onclick = async ()=>{
      const id = b.dataset.id;
      const r = byId.get(String(id)) || await repGet(id);
      if (!r) return;
...
function openReportViewer(r){
  ...
  const dt = r.createdAt ? new Date(r.createdAt) : null;
  const when = dt ? dt.toLocaleString("es-CO") : "—";
  const size = fmtBytes(r.sizeBytes || r.blob.size || 0);
  meta.textContent = `${when} · ${size}`;
  ...
  if (btnShare){
    btnShare.onclick = async ()=> {
      ...
      if (!ok) alert("No pude abrir el panel de compartir. Usa Descargar.");
    };
  }
  if (btnDown){
    btnDown.onclick = ()=> {
    ...
    if (IS_ANDROID){
      ...
      noPrev.innerHTML = `
        <div class="card">
          ...
          <div class="muted" style="margin-top:6px">En Android, el PDF no se puede mostrar embebido dentro de Logi. Ábrelo para verlo completo.</div>
...
(async function migrateFromRFIfNeeded(){
  ...
  if (document.getElementById("btnReportsRefresh")){
    document.getElementById("btnReportsRefresh").onclick = () => {
      ...
      try{ _reportsCache.delete(pid); }catch(e){}
      _reportsLoading = false;
      if (viewMode === "informes") render();
    };
  }
